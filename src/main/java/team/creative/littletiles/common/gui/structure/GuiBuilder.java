...
public class GuiBuilder extends GuiLayer {
    ...
    public final GuiSyncLocal<CompoundTag> CRAFT = getSyncHolder().register("craft", nbt -> {
        if ((getPlayer().isCreative() && builder.inventory.getItem(0).isEmpty())) {
            ...
            if (type != null) {
                ...
                if (!stack.isEmpty()) stack.setTag(LittleGroup.save(type.construct(grid, width, height, thickness, block.defaultBlockState())));
            }
        }
    });
    ...
    public void create() {
        ...
        add(new GuiStateButtonMapped<>("grid", LittleGrid.mapBuilder()));
        ...
        add(new GuiLeftRightBox().addLeft(new GuiLabel("failed").setTranslate("gui.structure_builder.failed").setVisible(false)).addRight(new GuiButton("craft", x -> {
            if ((getPlayer().isCreative() && builder.inventory.getItem(0).isEmpty())) {
                ...
                GuiComboBoxMapped<LittleStructureBuilderType> type = get("type");
                nbt.putString("type", type.getSelected().type.id);
                GuiStateButtonMapped<LittleGrid> gridButton = get("grid");
